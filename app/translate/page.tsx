...
export default function Page() {
  ...
  async function onOrigTextChange(value: string) {
    ...
    if (value !== "") return debouncedEventHandler({ sl: sourceLang, tl: targetLang, q: value });
  ...
  async function onSourceLanguageChange(value: string) {
    ...
    if (value === targetLang) setTargetLang(oldSourceLang);

    if (origText !== "") return translate({ sl: value, tl: targetLang, q: origText });
  }
  ...
  async function onTargetLanguageChange(value: string) {
    ...
    if (value === sourceLang) setSourceLang(oldTargetLang);

    if (origText !== "") return translate({ sl: sourceLang, tl: value, q: origText });
  }
  ...
  return (
    <div className="d-block w-100 vh-100 p-3">
      <div className="h-100 d-flex flex-column justify-content-center align-items-center">
        <div className="display-6 text-center mb-4">Google Dịch</div>
